...
// * В JS можно делать использовать математические выражения
// ! На примере мы делаем все мат функции и выводим их в консоль
let x;
x = 3 / 5 * 18 + 53 / 2 ** 3 - 2.424999999999997;
console.log(`Результат примера: ${x}`);
let y;
y = 50 % 17;
console.log(`Результат взятия остатка от деления: ${y}`);
let z = 58 - 'Ahbkfycth';
console.log(z);
...
//! Оператор  ||
// ! Выводим в консоль правду или лож, выводитс большее значение
console.log(1 || 0);
console.log(true || 'false');
console.log(null || 58);
console.log(null || 'фрилансер' || 0);
// ! Тут выводи последнее значение, т.к. всё не имеет значения
console.log(undefined || '' || 0 || null);
// ! Присвоение значения в переменную 
let putinName = '';
let putinNikName = 'Путин';
let putin = putinName || putinNikName || "Без имени";
console.log(putin);
// ! Сокращённое вычисление
let admins = 0; // Если заменить на 10, - выведет в консоль 5
let putins = 5;
admins > putins || putins++;
console.log(putins);


// ! Оператор &&
/*   Выводим в консоль true.
    У оператора && приоритет больше, чем у ||, поэтому первые действия
    это 1 && 0  и 2 && 1, где 0 - false, а 2 и 1 - true, но 1 написана
    в конце, поэтому консоль выводит её. Далее производится работа
    оператора ||, где между 0 и 1 выбирается true (1) и она выводится
    в консоль. */
console.log(1 && 0 || 2 && 1);

// ! Оператор ! (НЕ)
// ! Данный оператор является первым при выполнении
